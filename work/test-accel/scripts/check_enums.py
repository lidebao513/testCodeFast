...
def _iter_source_py():
    """产出需扫描的源码 .py（backend + scripts；排除门禁/脚手架自身与 enums.py 定义）。

    与 main() 的正向扫描口径对齐：scripts/ 里的工具脚本（如 pull_code.py）同样是
    「枚举值的使用方」，其引用应计入反向校验，避免误报孤儿。
    """
...
def main():
    ...
    if violations:
        print(
            f"[check_enums] 发现 {len(hard)} 处裸枚举字面量（应改用 backend.core.enums）+ "
            f"{len(soft)} 处碰撞提示："
        )
        for rel, i, v, snippet, note in violations:
            tag = "!" if not note else "~"
            print(f"  {tag} {rel}:{i}  '{v}'  -> {snippet[:90]}{note}")
        print("\n建议：from backend.core.enums import ... 后改用 XxxEnum.VALUE.value")
        if hard:
            return 1
