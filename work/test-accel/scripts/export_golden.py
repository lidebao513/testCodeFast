...
#
# 用法：
#   python scripts/export_golden.py                       # 默认导出所有项目
#   python scripts/export_golden.py --project 2           # 只导出 project_id=2
#   python scripts/export_golden.py --out ../../backup/golden
# ============================================================================
...
DEFAULT_DB = os.path.join(ROOT, "data", "test_accel.db")
DEFAULT_OUT = os.path.abspath(os.path.join(ROOT, "..", "..", "backup", "golden"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for p in projects:
            pid = int(p["id"])
            slug = f"project_{pid}"
            proj_dir = os.path.join(out_dir, slug)
            entry: dict[str, Any] = {
                "project_id": pid,
                "name": p["name"],
